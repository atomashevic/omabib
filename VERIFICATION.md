...
## In-window chat with Claude Code and Codex (2026-09-17)

- **Recorded CLI streams:** `scripts/record_chat_fixtures.py` ran short scripted chats against an isolated library and saved the output of Claude Code 2.1.272 (`stream-json`: a read tool call, a Bash command, an image, a WebFetch denied through a stand-in permission tool, an interrupt, a follow-up) and Codex 0.154 (`exec --json`: an MCP read, a shell command, an image, an `add_note` write, a SIGINT). Personal details were scrubbed: home paths, installed skills, plugins and commands, and plan usage in rate-limit events. Findings that shaped the adapters: Claude sends `init` every turn, one assistant event per content block and `ToolSearch` plumbing calls, auto-approves read-only Bash, and reports an interrupt as `error_during_execution`. Codex sends whole messages without deltas and counts cached input inside `input_tokens`. Its `add_note` ran without asking, the gap Omabib's MCP approval gate closes.
- **Rust:** 57 tests pass, `cargo clippy --all-targets -- -D warnings` is clean. `tests/chat.rs`: both adapters replay the recordings (every event understood; hidden tools dropped; deltas add up to each message; six turns, one interrupted); argument building; the MCP gate (reads and PDF fetches pass, writes wait, a declined write never runs, Claude permission prompts answered in both directions); and the manager against fake Claude/Codex executables: streaming with deltas pushed but not stored, context and `mcp.json` written 0600 with the chat ID, one Claude process reused across turns, a quoted selection and a clip rendered from a real PDF sent as a PNG block, interrupt, one turn at a time, approvals allowed, answered once and expired, the terminal command without the approval queue, Codex resumed by thread with instructions sent once and a SIGINT turn, and deleting the reference removing chats, rows and folders.
- **Node:** `scripts/test_chat_text.js` (4 tests: tool labels, approval text, page links outside tags and existing links, turn lines).
- **Offscreen QML:** 25 tests pass on software and OpenGL. The chat test renders user, session, tool (with its result merged), assistant, turn and approval rows, checks the page link, the streaming draft, the approval buttons and their disappearance once answered, Enter sending only when idle, and the selection chip. The reader test checks `c` attaching the selection. The screenshots showed two bugs, both fixed and re-checked: page links in Qt's default blue, and the busy state (status line, typing dots, Stop) not updating when a chat changed in place. A focus bug was also fixed: Escape from the side pane's chat restored focus to the chat itself, because the reader's focus scope remembers the last focused child. The reader now has `focusPages()`.
- **Not run:** `scripts/test_chat_agents.py` (real agents through the service, a read and an approved and a denied write), which was stopped before it started; the installed UI with real agents; and `scripts/test_tabs_ui.py`.
- **Installed (2026-09-17):** `./scripts/install.sh` and `omarchy restart shell` with Omabib closed. The shell log shows no QML errors, `state` reports the new `chat` key (default agent Codex, from Settings), both saved tabs were restored, the user service is active, and on the real library `chat_list` answers (no chats yet) and `chat_subscribe` confirms. No chat was sent.

## Chat: Omabib tools in Codex, agent steps, model picker, citations (2026-09-17)

- **Why `get_reference` was "unavailable":** the first real Codex chat never called Omabib's MCP tools. Codex 0.154 runs tools through a JavaScript `exec` tool and leaves MCP tools out of its description until the model searches `ALL_TOOLS`, so the model followed the installed skill to `omabib call get_reference`. That command failed inside Codex's read-only sandbox with `Operation not permitted`, which the CLI reported as "service unavailable". To confirm the MCP side works under the user service, Codex was pointed (via `model_provider`) at a local stand-in Responses API that scripted one `exec` call. No subscription was used. Run with `systemd-run --user -p NoNewPrivileges=true`, like the service, the call listed all 19 `mcp__omabib__*` tools and `tools.mcp__omabib__get_reference` returned the real entry.
- **Fix:** the chat instructions now name the tools per agent (for Codex: call `tools.mcp__omabib__…`, filter `ALL_TOOLS`, don't use the `omabib` command) and ask for "p. N" instead of file citations; the shipped skill says the same. Inside `codex sandbox` (read-only), `omabib call` now prints "Omabib's socket is blocked here (Operation not permitted…); this is likely a sandbox. Use Omabib's MCP tools instead." The block itself stays: a CLI that worked in the sandbox would skip the approval queue. `omabib schema get_reference` prints one tool (it failed before, which broke Codex's combined command).
- **Rust:** 58 tests pass, clippy clean, chat files rustfmt-clean. New checks: `--model/--effort` for Claude Code and `-m`/`model_reasoning_effort` for Codex; Codex catalog parsing (hidden models dropped, priority order) and the top-level defaults of `config.toml` (profile tables ignored); `chat_models` for both agents against a fake `codex debug models` and temporary config dirs; invalid efforts and model names refused; `chat_set_model` making the next Claude message relaunch the process with `--resume … --model opus --effort high`, without an error event; the terminal command carrying the model; Codex turns carrying it; an older library gaining the `model`/`effort` columns on open.
- **Real CLIs, no model calls:** against an isolated service, `chat_models` listed Codex's five listed models with their effort levels and the configured default (`gpt-5.6-sol`, `high`), and Claude Code's aliases with `opus` from `settings.json`.
- **Node:** `test_chat_text.js` 5 tests; the new one covers file citations to links (URL-encoded paths, `page_number`), dropped `codex-annotation`, a half-streamed directive hidden, and plain text for Copy.
- **Offscreen QML:** 25 tests pass on software and OpenGL. The chat test checks that the session line, a message written before a tool call and the call are zero-height with steps hidden and shown after the header toggle; the citation renders as `sparse.pdf` with an `omabib-file:` link; Copy sits in the turn footer and copies without the directive; the model chip names the default model; the menu picks a model with the keyboard (the menu cursor now starts on the first selected entry, since this menu selects one per section); the activity line shows the running command. Screenshots checked: paper-tab chat with steps hidden and shown, and the reader side pane's narrower header (agent icon only).
- **Settings:** `scripts/test_claude.py` passes with `chat_steps` and per-agent model and effort keys, several set in one write, bad values refused.
- **Installed (2026-09-17):** `./scripts/install.sh` and `omarchy restart shell` with Omabib closed and no chat running. The user service is active; the shell log has no Omabib errors (only an unrelated Omamail warning); `state` shows the chat defaults (Codex, default model, steps hidden) and both saved tabs restored; the real library gained the `model` and `effort` columns; `chat_models` answers for both agents; the installed skill has the new MCP guidance.
- **Not run:** a real Codex or Claude Code turn with the new instructions, and the installed UI.

## Cloud sync implementation review and automated verification (2026-09-18)

- **Documentation and shipped skill:** `README.md`, `API.md`, and `skills/omabib/SKILL.md` now describe two-way folder/rclone sync, the service operations, `source:"cloud"` PDF retrieval, local-only chats and note revisions, and the implemented duplicate behavior. The retired repository, Git LFS, and one-way-history instructions were removed. The old installed `omabib-history` command remains mentioned only in `install.sh`, where installation removes that managed obsolete copy.
- **Review cleanup:** removed temporary multi-computer failure diagnostics from `tests/sync.rs`; retained a compact canonical-dump comparison. `git diff --check` passes. No `DEBUG`, `TODO`, or `FIXME` marker remains in `src/sync/` or `tests/sync.rs`. The schema output now lists every non-MCP sync operation.
- **Rust:** scoped rustfmt check passes for `src/sync/*.rs` and `tests/sync.rs`. `cargo test --locked` passes: 24 unit, 6 chat, 29 library, 1 math, 2 PDF, 4 sync, and 2 visual tests (68 total). The sync tests cover folder and rclone-local backends, PDF transfer, joining, merging a local-only library, same-DOI/citekey convergence, snapshot joining, and randomized three-computer convergence. `cargo clippy --all-targets -- -D warnings` is clean, and `cargo build --release --locked` succeeds. A repository-wide rustfmt check is not used because unrelated pre-existing Rust files are not rustfmt-clean.
- **Scripts and plugin:** `scripts/test_transport.py`, `scripts/test_claude.py`, and `scripts/test_codex.py` pass against `target/release/omabib`. The overview, Markdown, and chat JavaScript suites pass (10, 9, and 5 tests). `omarchy plugin validate plugin` passes.
- **Offscreen QML:** 26 tests pass with both the software renderer and OpenGL. The Sync test covers storage choice, folder entry, browser authorization, joining, merge/replace choice, new-library progress, configured status, PDF download, and conflict actions. Screenshots `14a-sync-choose.png`, `14c-sync-connecting.png`, and `14g-sync-status.png` were visually inspected: the dialog fits without clipping, hierarchy and status text are readable, and conflict actions are visible. The software renderer skips only the existing PDF theme pixel check; OpenGL runs it.
- **Still intentionally not run:** no real cloud account was connected, no real library was uploaded or joined, the source tree was not installed, and Omarchy was not restarted. Google Drive remains blocked until the Omabib OAuth client is registered or `OMABIB_GOOGLE_CLIENT_ID` is supplied. No subscription-backed chat-agent test or live keyboard-driving UI test was run.

## Google Drive OAuth connection (2026-09-18)

- Created Google Cloud project `omabib-sync` and the Omabib Desktop OAuth client. The user's account is registered as a test user; the app remains in Testing.
- Added loading of Google's desktop client JSON from `google-client.json` beside Omabib's rclone config, preserving environment-variable overrides. The local client JSON and rclone token config both have mode `0600`; neither is in the repository.
- A debug build running against an isolated empty library completed the real browser OAuth flow with only `https://www.googleapis.com/auth/drive.file`. The browser showed Success; `sync_status` reported `connected:true` and authorization state `done`. A direct authenticated rclone root listing succeeded with exit code 0 and no app-visible files. No shared library has been initialized or uploaded.
- Debug build, the connection parser unit test, Clippy with warnings denied, and diff whitespace checks passed. The installed service and real library remain unchanged; the new credential loader still needs deployment with cloud sync.

## Cloud sync installation (2026-09-18)

- Created a consistent database backup at `~/.local/share/omabib/before-cloud-sync-install-20260918.db`, then ran `scripts/install.sh` and `omarchy restart shell` with Omabib closed and no active editor or chat.
- The release build and installation succeeded. The installed binary matches the release artifact, plugin validation passes, and the restarted service is active. Shell IPC reports the new sync state and no application error. Google Drive is available through the installed service; both credential files retain mode `0600`.
- The real library remains unconnected with sync off and all 16 PDFs local. No upload was started. The installer removed the obsolete managed `omabib-history` command; the history checkout and remote repository were untouched.

## Batched cloud uploads and duplicate-sync status (2026-09-18)

- PDFs and clips are staged privately in batches of eight and sent with one `rclone copy` process per batch, four transfers and four checkers. Checksums and immutable-destination checks remain enabled. No remote deletion is used. Folder storage keeps its atomic per-file copy path. Queue entries are removed only after a successful batch; staging is automatically cleaned up.
- `sync_now` acquires the cycle lock before changing status, progress, last-attempt or retry state. A concurrent call returns `already_running:true`; the UI shows an informational message rather than claiming success or setting a sync error.
- All 71 Rust tests pass, including new tests for duplicate-request status preservation, failed-batch queue retention and retry across batch boundaries, and real rclone-local nested uploads, repeated upload, unrelated-file preservation and immutable conflict rejection. Clippy is clean, plugin validation passes, and all 26 software-rendered QML tests pass.
- The user's original Google Drive upload finished with zero pending files before deployment of this change. The new upload path has been tested against local rclone storage; a real Google Drive throughput comparison has not been performed.
- Installed with `scripts/install.sh` after the release build passed, with Omabib closed and no active editor/chat. The installed binary matches the release artifact; the restarted service is active and plugin validation passes. A real Drive sync completed successfully. A second concurrent `omabib sync --json` returned `already_running:true` while status stayed `syncing` with an empty error message; the first returned `already_running:false` normally. No pending files remained.
